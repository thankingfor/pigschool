 ...
 */
function selectPic2(){
	$('#filePicture2').fileinput('upload'); //触发插件开始上传。
	//异步上传返回结果处理 多个图片每次独立回调
	$("#filePicture2").on("fileuploaded", function (event, data, previewId, index) {
		//alert(JSON.stringify(event));alert(JSON.stringify(data));alert(JSON.stringify(previewId));alert(JSON.stringify(index));
		var imgUrls = data.response.urls;
		$('#addFormPicName2').append("<img src='"+imgUrls+"' style='height: 50px;width: 70px;'>");
		//给<input type="hidden" id="addFormImage" name="image" class="form-control">赋值
		if($('#addFormPic2').val() == ""){
			$('#addFormPic2').val(imgUrls);
		}else{
			//用,号区分
			$('#addFormPic2').val($('#addFormPic2').val()+","+imgUrls);
		}
	 });
	 $("#selectPicModal2").modal("hide");
}
...
function selectCat(){
	...
	//给<input type="hidden" id="addFormCategoryId" name="categoryId" class="form-control" >id
	$('#addFormCategoryId').val(node.id);
	//关闭模态框selectCatModal
